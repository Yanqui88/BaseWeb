...
export default function AdminShell({ children, tenantName, tenantDomain, tenantSlug, userName, userEmail }: AdminShellProps) {
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);

  const slug = tenantSlug || (tenantDomain && !tenantDomain.includes('.') ? tenantDomain : 'demo');
  const storeHref = tenantDomain && tenantDomain.includes('.') && !tenantDomain.includes('localhost')
    ? (tenantDomain.startsWith('http') ? tenantDomain : `http://${tenantDomain}`)
  ...
  return (
    <div className="flex min-h-screen bg-zinc-950 text-zinc-100 relative w-full">
      ...
      >
        <Sidebar onCloseMobile={() => setMobileMenuOpen(false)} userName={userName} userEmail={userEmail} />
      </div>
        ...
        <header className="sticky top-0 z-20 flex items-center justify-between px-4 md:px-8 h-16 bg-zinc-950/80 backdrop-blur-md border-b border-zinc-800/60">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-zinc-900 border border-zinc-800 text-xs font-medium text-zinc-400">
              ...
              <span className="text-zinc-200 font-semibold">{tenantName || 'Acme Corporation'}</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            {/* Ver mi Tienda Button */}
            <a
              href={storeHref}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 text-zinc-200 hover:text-white text-xs font-medium transition-colors duration-200 cursor-pointer active:scale-95"
            >
